...
class ScryptCalc(object):
    ...
    def base58_encode(input_bytes:bytes)->str:
        quotient=int.from_bytes(input_bytes,byteorder="big")
        encoded_data=""
    
        while quotient>0:
            quotient,remainder=divmod(quotient,58)
            encoded_data=f"{ScryptCalc.BASE58_CHARACTERS[remainder]}{encoded_data}"
    
        quotient=0
        remainder=0
        leading_zeros=(len(input_bytes)-len(input_bytes.lstrip(b"\x00")))
        return_value=f"{'1'*leading_zeros}{encoded_data}"
        encoded_data=""
        leading_zeros=0
        return return_value
            ...
            while self.request_exit.is_set()==False:
                ...
                if self.result_updated.is_set()==True:
                    ...
                    for result_character_index in range(result_length):
                        ...
                        keyboard.write(current_character,delay=0,exact=True,restore_state_after=False)
        ...
        class Main_Window(QMainWindow):
            ...
            def display_result(self):
                ...
                if result_format=="bin" and len(self.result_bytes)>0:
                    ...
                    text_value=ScryptCalc.base58_encode(self.result_bytes)
